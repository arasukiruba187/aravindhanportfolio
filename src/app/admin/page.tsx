...
export default function AdminPage() {
  ...
  const [projects, setProjects] = useState<ProjectEntry[]>(
    Array.from({ length: 5 }, () => ({ title: "", videoUrl: "", thumbnailUrl: "", category: "Featured Work" }))
  );
  ...
  useEffect(() => {
    ...
    fetch("/api/config")
      ...
      .then(data => {
        ...
        const mapped: ProjectEntry[] = Array.from({ length: 5 }, (_, i) => ({
          title: items[i]?.title ?? `Project ${i + 1}`,
          videoUrl: items[i]?.videoUrl ?? items[i]?.rawUrl ?? "",
          thumbnailUrl: items[i]?.thumbnailUrl ?? "",
          category: items[i]?.category ?? "Featured Work",
        }));
        setProjects(mapped);
      })
                ...
                {projects.map((proj, idx) => (
                  <div key={idx} style={{ padding: "1.5rem", border: "1px solid rgba(255,255,255,0.06)", background: "#111", display: "flex", flexDirection: "column", gap: "1rem" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.25rem" }}>
                      ...
                      <span style={{ fontFamily: "Space Mono,monospace", fontSize: 9, color: "rgba(240,240,240,0.35)", letterSpacing: "0.2em", textTransform: "uppercase" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                      <div style={fieldStyle}>
                        ...
                        <input type="text" value={proj.title} onChange={e => updateProject(idx, "title", e.target.value)}
                          placeholder={`e.g. Brand Campaign ${idx + 1}`} style={inputStyle}
                          onFocus={e => (e.target.style.borderColor = "#e63946")}
                          onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
                      </div>
                      <div style={fieldStyle}>
                        ...
                        <select value={proj.category} onChange={e => updateProject(idx, "category", e.target.value)}
                          ...
                          onFocus={e => (e.target.style.borderColor = "#e63946")}
                          onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.1)")}>
                          {["Featured Work", "Commercial Projects", "Viral Content", "YouTube Content", "Reels", "Color Grading"].map(c => (
