...
export default function Hero({ tagline, subtagline, items, activeVideoUrl, onVideoChange }: HeroProps) {
  ...
  useEffect(() => {
    if (timelineItems.length > 0) {
      const initialPercent = (activeIndex / timelineItems.length) * 100 + (100 / timelineItems.length) / 2;
      setPlayheadX(initialPercent);
    }
  }, [timelineItems, activeIndex]);

  const handleTimelineHover = (idx: number, item: PortfolioItem) => {
    setActiveIndex(idx);
    if (item.videoUrl) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="a-timeline-wrapper relative flex flex-col w-full z-20">
                  ...
                  {timelineItems.map((item, idx) => {
                    ...
                    return (
                      <motion.div
                        ...
                        transition={{ duration: 0.65, delay: 0.45 + idx * 0.08, ease: [0.16, 1, 0.3, 1] as const }}
                        style={{
                          left: `${leftPercent}%`,
